...
class CategoryController extends AbstractController
{
    ...
    public function delete(Category $category, Request $request, CategoryRepository $categoryRepository): Response
    {
        ...
        $category = $categoryRepository->find($cateId);
        if (!$category) {
            throw $this->createNotFoundException('No news found for id ' . $cateId);
        }
        $form = $this->createFormBuilder($category)->add('delete', 'submit')->getForm();
        $form->handleRequest($request);
        if ($form->isValid()) {
            $em->remove($category);
            $em->flush();
        ...
        return $this->render('category/create.html.twig', $build);
    }
}
